...
import { QaAnswersPage } from "../qa-answers/qa-answers";
...
@IonicPage()
@Component({
  selector: 'page-qa-deatils',
  templateUrl: 'qa-deatils.html',
})
export class QaDeatilsPage extends BaseUI {
  ...
  showQaAnswersPage() {
    let modal = this.modalCtrl.create(QaAnswersPage, { "id": this.id });
    //关闭后的回调
    modal.onDidDismiss(() => {
      this.loadQuestion(this.id);
    });
